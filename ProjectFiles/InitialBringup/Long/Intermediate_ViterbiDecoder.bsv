...
method Bool done();
  ...
  rule rl_TRACEBACK_LOOP(currState == TRACEBACK_LOOP);
    ...
    OutputMatrixAddr outAddr = zeroExtend(traceBackTime - 1);
  ...
  rule rl_TRACEBACK_WRITE_FINAL_PROB(currState == TRACEBACK_WRITE_FINAL_PROB);
    ...
    OutputMatrixAddr lastAddr = zeroExtend(timeStep - 1);
    ...
    // Almost there - Issue read for next input
    readReqFIFO_Input1.enq(MemReadReq{addr: inputIndex});
    currState <= CHECK_FOR_NEXT_SEQ;

  endrule : rl_TRACEBACK_WRITE_FINAL_PROB

  rule rl_CHECK_FOR_NEXT_SEQ(currState == CHECK_FOR_NEXT_SEQ);
    $display("[DUT] Time=%0t: Current State = ", $time, fshow(currState));
    // Reading and checking to see if next value is 0000 or something else
    // 0000 - indicates end of file
    // something else - indicates next sequence
    let respInput <- readRespFIFO_Input1.first;
    readRespFIFO_Input1.deq;
    if (respInput.data == 32'h00000000) begin
      ...
      $display("DEBUG : [DUT] Time=%0t: Another sequence detected. Do the same thing. Again.", $time);

    end    

  endrule : rl_CHECK_FOR_NEXT_SEQ
